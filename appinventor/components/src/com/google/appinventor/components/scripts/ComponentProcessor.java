...
public abstract class ComponentProcessor extends AbstractProcessor {
  ...
  private static final Set<String> SUPPORTED_ANNOTATION_TYPES = ImmutableSet
      .of("com.google.appinventor.components.annotations.DesignerComponent",
  ...
  private void processComponent(Element element) {
    ...
    if (!directSupertypes.isEmpty()) {
      ...
      // Otherwise, we need to copy its designer properties, properties,
      // methods, and events.
      if (parentComponent != null) {
        // Copy its permissions, designer properties, properties, methods, and
        // events.
        componentInfo.permissions.addAll(parentComponent.permissions);
        // Since we don't modify DesignerProperties, we can just call Map.putAll
        // to copy the
